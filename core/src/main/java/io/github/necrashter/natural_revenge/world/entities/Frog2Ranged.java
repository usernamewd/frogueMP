...
public class Frog2Ranged extends FrogBase {
    PursueEntity pursueEntity;
    SpitOnce spitOnce;
    public Frog2Ranged(GameWorld world) {
        super(world);
    ...
    class SpitOnce extends State {
        ...
        public void init() {
            ...
            movement.setZero();
            animationController.setAnimation("spit", -1,2f, animationListener);
            attackDelta = 0f;
            spitted = false;
        }
        ...
        private void spit() {
            ...
            f.ignoredEntity = Frog2Ranged.this;
            f.position.set(hitBox.position).mulAdd(forward, .25f).add(0f, .5f, 0f);
            f.velocity.set(world.player.hitBox.position).add(0f, 1.2f, 0f).sub(f.position).nor().scl(20f);
            f.spawn();
            world.octree.add(f);
        }
